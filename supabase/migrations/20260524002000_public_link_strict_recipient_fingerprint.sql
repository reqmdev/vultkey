alter table public.public_key_claims
  add column if not exists recipient_device_hash text,
  add column if not exists recipient_browser_hash text,
  add column if not exists recipient_request_hash text;

create index if not exists public_key_claims_link_device_idx
on public.public_key_claims (link_id, recipient_device_hash)
where recipient_device_hash is not null
  and status in ('reserved', 'redeemed');

create index if not exists public_key_claims_link_browser_idx
on public.public_key_claims (link_id, recipient_browser_hash)
where recipient_browser_hash is not null
  and status in ('reserved', 'redeemed');

create index if not exists public_key_claims_link_request_idx
on public.public_key_claims (link_id, recipient_request_hash)
where recipient_request_hash is not null
  and status in ('reserved', 'redeemed');

create index if not exists public_key_claims_link_ip_idx
on public.public_key_claims (link_id, ip_hash)
...
begin
  ...
  if v_max_per_recipient > 0 then
    ...
    if p_recipient_device_hash is not null and (
      select count(*)
      from public.public_key_claims c
      where c.link_id = v_link.id
        and c.recipient_device_hash = p_recipient_device_hash
        and c.status in ('reserved', 'redeemed')
    ) >= v_max_per_recipient then
      ok := false;
      message := 'Bu cihaz bu linkten daha fazla kod alamaz.';
      return next;
      return;
    end if;

    if p_recipient_browser_hash is not null and (
      select count(*)
      from public.public_key_claims c
      where c.link_id = v_link.id
        and c.recipient_browser_hash = p_recipient_browser_hash
        and c.status in ('reserved', 'redeemed')
    ) >= v_max_per_recipient then
      ok := false;
      message := 'Bu tarayıcı bu linkten daha fazla kod alamaz.';
      return next;
      return;
    end if;

    if p_recipient_request_hash is not null and (
      select count(*)
      from public.public_key_claims c
      where c.link_id = v_link.id
        and c.recipient_request_hash = p_recipient_request_hash
        and c.status in ('reserved', 'redeemed')
    ) >= v_max_per_recipient then
      ok := false;
      message := 'Bu tarayıcı/ağ bu linkten daha fazla kod alamaz.';
      return next;
      return;
    end if;
    ...
    if p_ip_hash is not null and (
      ...
      ok := false;
      message := 'Bu ağ bu linkten daha fazla kod alamaz.';
      return next;
      return;
    end if;
