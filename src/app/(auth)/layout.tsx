    ...
    <main className="relative isolate flex min-h-screen flex-col overflow-x-hidden bg-background text-foreground">
      ...
      <header className="relative z-10 border-b border-border bg-background/88">
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <Link href="/" className="hidden h-10 items-center gap-2 rounded-md border border-border bg-background/70 px-3.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground sm:inline-flex">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl flex-1 items-center gap-10 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,0.96fr)_minmax(380px,460px)] lg:px-8 lg:py-12">
        <section className="hidden max-w-xl lg:block">
          ...
          <h1 className="max-w-lg text-4xl font-semibold leading-tight tracking-[-0.035em] text-foreground">Key kasana gir, dağıtımı kaldığın yerden yönet.</h1>
          <p className="mt-4 max-w-lg text-sm leading-6 text-muted-foreground">
            Vultkey, dijital keyleri ve oyun kodlarını saklamak, paylaşmak ve durumlarını temiz tutmak için sade bir çalışma alanı sunar.
          </p>
          <ol className="mt-8 grid max-w-lg gap-2 rounded-lg border border-border/70 bg-card/55 p-3">
            {authSteps.map(({ step, title, text, icon: Icon }) => (
